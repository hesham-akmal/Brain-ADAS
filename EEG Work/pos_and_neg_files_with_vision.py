...
def make_columns_drawing(electrodes_and_vision):
    newCols = []
    for col in electrodes_and_vision:
        for i in range(192):
            newCols.append(col + '.' + str(i))
    newCols.append('y')
    return newCols

def convert_to_row(start, label, electrodes_and_vision, data):
    row = []
    for col in electrodes_and_vision:
...
def positive_events(newCols, y, data, electrodes_and_vision):
    ...
    while j < (len(y)):
        ...
        if (y[j] != 0):
            #IMPORTANT: CHECK IF DRIVER BRAKE IS REMOVED BECAUSE PEDALS DON'T WORK
            ...
            #    j+=1
            if (j+192 < len(y)):
                posDfs.loc[len(posDfs)] = convert_to_row(j, 1, electrodes_and_vision, data)
                indices.append(j)
                j += 384 #192*2
            else:
                j+=1
...
def negative_events(indices, newCols, y, data, electrodes_and_vision):
    ...
    while j < (len(y)):
        if(len(indices) > 0 and j <= indices[0] and j+192 > indices[0]):
            ...
            negDfs.loc[len(negDfs)] = convert_to_row(j, 0, electrodes_and_vision, data)
            j += 192
        else:
            break
    return negDfs
...
def upload_and_draw(folder, preprocess, apply_filter, fc):
    ...
    data = data.drop(["COUNTER", "timestamp", "EEGprob", "SystemBrake"], axis = 1)
    for i in range(1,len(fnames)):
        d = upload_file(folder + "/" + fnames[i])
        d = d.drop(["COUNTER", "timestamp", "EEGprob", "SystemBrake"], axis = 1)
        data = data.append(d, ignore_index=True)
        print(fnames[i])
    electrodes_and_vision = data.columns.drop([data.columns[0], "y"])
    newCols = make_columns_drawing(electrodes_and_vision)
    y = data["y"]
    posDfs, indices = positive_events(newCols, y, data, electrodes_and_vision)
    posDfs.to_csv("Our Dataset/csv/"+folder+"pos.csv", index = False)
    negDfs = negative_events(indices, newCols, y, data, electrodes_and_vision)
