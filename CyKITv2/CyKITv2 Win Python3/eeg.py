...
#  ControllerIO().  I/O threaded bridge to browser (for CyWebSocket.py and
#  eeg.py)
# ¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯
class ControllerIO():
...
def settings_menu(device, sIO, intf):
    ...
    if mode_select.isdigit() == True:
        ...
        if mode_select > -1 and mode_select < 9:
            ...
            #2 EPOC+ 128hz 16bit - MEMS 32hz 16bit 0x86 (d.134) 55 AA 20 12 86
            #IN 55 AA 88 12 00
                ...
                if eeg_driver == "pywinusb":
                    ...
                    while changed_mode != EPOC_ChangeMode:
                        ...
                        for inputReport in successReport:
                            ...
                            #using STR comparison instead of built-in SET due
                            #to py error.
...
class EEG(object):
        ...
        if(CalypsoReceive_BADASbool):
            try:
                ...
                if((str(e)).find('cannot find the file') != -1 ):
                    print('tiva_receive COM port not connected')
            ...
            try:
                ...
                if((str(e)).find('cannot find the file') != -1 ):
                    print('tiva_send COM port not connected')
                if((str(e)).find('PermissionError') != -1):
                    print('tiva_send COM port serial already opened')
        ##################################################################################################################################################################
    ...
    def start(self):
        ...
        #BADAS
        if(CalypsoReceive_BADASbool):
            #Run loop receiving from calypso in thread
    ...
    def Setup(self, model, config):
        ...
        # 'EPOC BCI', 'Brain Waves', 'Brain Computer Interface USB
        # Receiver/Dongle', 'Receiver Dongle L01'
                        ...
                        if all_devices.product_name == deviceList[i]:
                            ...
                            if useDevice.upper() == "Y":
                                ...
                                if all_devices.product_name == "EPOC+":
                                    ...
                                    #self.cyIO.setReport("Device",
                                                                                               #all_devices.find_output_reports())
            ...
            while not tasks.empty() and self.running == True:
                ...
                try:
                    ...
                    if eeg_driver == "bluetooth":
                        ...
                        if BTLE_device_name == "Insight":
                            data = task[19:20] + sel_decrypt + task[16:17] + task[17:18] + task[18:19]
                                ...
                                ##################################################
                                    ...
                                    if(cyIO.DriverBADAS != None): #Vision thread running 
                                        if( (cyIO.visionThread.prob + EEGprob)/2 > 0.5 ):
                                            print('Simulation of calypso algo + EEG and No vision :: Brake')
                                            self.cyIO.DriverBADAS.EmergencyEventSeq()
                                        else:
                                            print('Simulation of calypso algo + EEG and No vision :: No Brake')
                                            self.cyIO.DriverBADAS.BrakeSystemUpdate()

                                    else: #No vision thread running ( Only EEG prediction )
                                        if( EEGprob > 0.5 ):
                                            print('Simulation of calypso algo + EEG and vision :: Brake')
                                            self.cyIO.DriverBADAS.EmergencyEventSeq()
                                        else:
                                            print('Simulation of calypso algo + EEG and vision :: No Brake')
                                            self.cyIO.DriverBADAS.BrakeSystemUpdate()
                                ##################################################

                                print('EEGprob: ' , EEGprob)
                                print('cyIO.visionThread.prob: ' , cyIO.visionThread.prob)
                                cyIO.CurrentPacket = 'X,' + counter_data + packet_data + self.delimiter + 'X,' + str(EEGprob) + self.delimiter + str(cyIO.visionThread.prob)
                                ...
                                if (int(time.time() % 60) - self.baseSeconds) == 1:  # Baseline Every Second.
                                    ...
                                    try:
                                        ...
                                        if baseline_values != None and 'baseline_last' in locals():
                                            ...
                                            # ¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯
                                            # self.epocOrder =
                                            # [2,3,0,1,4,5,6,7,8,9,12,13,10,11]
                                            #baseline_new = [baseline_values[i]
                                            #for i in self.epocOrder]
                    ...
                    try:
                        if self.openvibe == True:
                            if self.integer == True:
                                cyIO.sendOVint(packet_data)
                            else:
                                cyIO.sendOVfloat(packet_data)
